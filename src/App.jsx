    ...
    if (this.state.hasError) {
      return (
        <div className="p-8 bg-red-900 text-white min-h-screen flex flex-col items-center justify-center text-center">
          ...
          <p className="mb-4 text-sm opacity-80">請截圖此畫面給開發者</p>
          <div className="bg-black p-4 rounded overflow-auto w-full max-w-md text-xs font-mono border border-red-500 text-left">
            {this.state.error?.toString()}
          </div>
          <button onClick={() => window.location.reload()} className="mt-8 px-6 py-3 bg-white text-red-900 rounded-full font-bold">重新整理 (Reload)</button>
        </div>
      );
    }
...
const translations = {
    ...
    // 其他語言省略以節省空間，系統預設 fallback 到 en
};
...
const App = () => {
    ...
    const fetchEvents = useCallback(async () => {
        ...
        try {
            ...
            setEvents(snap.docs.map(d => ({ id: d.id, ...d.data(), categories: d.data().categories || ['Standard'], paymentLink: d.data().paymentLink || '' })));
    ...
    const fetchMyData = useCallback(async () => {
        ...
        onSnapshot(q, (snap) => {
            setMyRegistrations(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        });
    }, [user]);

    useEffect(() => { if(isAuthReady) { fetchEvents(); if(user) fetchMyData(); } }, [isAuthReady, user, fetchEvents, fetchMyData]);
        ...
        <div className="p-4 pb-24 space-y-4">
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    <select value={lang} onChange={e=>setLang(e.target.value)} className="bg-gray-800 text-white text-xs p-1 rounded"><option value="zh-TW">繁體</option><option value="en">EN</option></select>
                    <button onClick={handleLogout}><LogOut size={16} className="text-gray-400"/></button>
                </div>
            </div>
            {events.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(e => (
                <div key={e.id} onClick={() => navigate('detail', e)} className="bg-gray-800 p-4 rounded-xl mb-3 cursor-pointer relative overflow-hidden shadow-md border border-gray-700">
                    {e.bannerUrl && <img src={e.bannerUrl} className="absolute inset-0 w-full h-full object-cover opacity-30" alt=""/>}
                    <div className="relative z-10">
                        <h3 className="text-lg font-bold text-white">{e.name}</h3>
                        <div className="flex gap-1 mt-1 flex-wrap">
                            {e.categories?.map(c => <span key={c} className="text-[10px] bg-indigo-900 text-indigo-200 px-1 rounded border border-indigo-700">{c}</span>)}
                        </div>
                        <p className="text-sm text-gray-300 mt-2">{formatDateOnly(e.date)}</p>
                    </div>
                </div>
            ))}
            {events.length === 0 && <div className="text-gray-500 text-center mt-10">{t('noEvents')}</div>}
        </div>
    );
    ...
    const EventDetail = ({ event }) => {
        ...
        useEffect(() => {
            if (reg?.called) {
                ...
                if (typeof navigator !== 'undefined' && navigator.vibrate) {
                    try { navigator.vibrate([500, 200, 500, 200, 1000]); } catch(e) {}
                }
                // 🔥 安全音效
                if (audioRef.current) {
        ...
        const handleReg = async () => {
            if (!stageName) return alert("Enter Stage Name");
            try {
                const ref = collection(db, `artifacts/${appId}/public/data/registrations`);
                await addDoc(ref, { 
                    eventId: event.id, userId: user.uid, stageName, category: cat, 
                    registrationTime: serverTimestamp(), checkedIn: false, paid: false, 
                    laneAssignment: null, queueNumber: null 
                });
                alert(t('successMsg'));
                navigate('registerSuccess', { ...event, laneAssignment: null });
                ...
                {isEditing ? (
                    <form onSubmit={handleUpdate} className="bg-gray-800 p-4 rounded-xl space-y-3 border border-gray-700">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button type="submit" className="flex-1 bg-green-600 p-3 rounded font-bold">{t('saveChanges')}</button>
                        </div>
                    ...
                    <>
                        ...
                        <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 relative">
                            ...
                            {event.paymentLink && (
                                <a href={event.paymentLink} target="_blank" rel="noopener noreferrer" className="block w-full bg-green-600 hover:bg-green-500 text-white text-center font-bold py-3 rounded-xl mt-4 shadow-lg">
                                    <CreditCard size={20} className="inline mr-2"/> {t('payNowBtn')}
                                </a>
                            )}
                        </div>
                        <div className="fixed bottom-20 left-0 right-0 px-4 z-20">
                            {isCreator ? (
                                ...
                                <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-xl space-y-3">
                                    ...
                                    <button onClick={handleReg} className="w-full bg-red-600 p-4 rounded-xl font-bold shadow-lg flex justify-center">{t('randomRegisterBtn')}</button>
                                </div>
                            ) : (
                                <div className="bg-gray-900/90 p-4 rounded-xl border border-green-600 text-center shadow-xl backdrop-blur">
                                    ...
                                    <div className="flex justify-center gap-2 mt-2 text-xs">
                                        <span className={`px-2 py-1 rounded ${reg.checkedIn ? 'bg-green-600' : 'bg-gray-700'}`}>{reg.checkedIn ? t('statusCheckedIn') : t('statusNotCheckedIn')}</span>
                                        <span className={`px-2 py-1 rounded ${reg.paid ? 'bg-green-600' : 'bg-gray-700'}`}>{reg.paid ? t('statusPaid') : t('statusNotPaid')}</span>
                                    </div>
                                </div>
                            )}
                        </div>
